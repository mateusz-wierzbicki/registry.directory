      ...
      {entries.map((entry, index) => {
        ...
        return (
          <div key={encodeURIComponent(entry.url)} className="h-full">
            <Card className="bg-black border border-neutral-700/50 rounded-none overflow-hidden shadow-none hover:shadow-lg transition-shadow h-full flex flex-col relative">
              {affiliate && (
                <span className="absolute top-0 right-0 text-[9px] font-mono uppercase tracking-wider text-neutral-500 bg-neutral-900 border-b border-l border-neutral-700/50 px-1.5 py-0.5 z-10">
                  Sponsored
                </span>
              )}
